...
func TestReverseProxyCreateResponse(t *testing.T) {
	...
	director := rproxy.Director
	rproxy.Director = func(req *http.Request) bool {
		director(req)
		return req.URL.Path == "/mod"
	}
	...
	tests := []struct {
		...
		{frontendProxy.URL + "/schedule", http.StatusForbidden},
	}
...
func TestReverseProxyErrorHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name: "createresponse_noerr",
			director: func(req *http.Request) bool {
				return false
			},
			createResponse: func(rw http.ResponseWriter, req *http.Request) error {
				rw.WriteHeader(346)
				return nil
			},
			errorHandler: func(rw http.ResponseWriter, req *http.Request, err error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.director != nil {
				rproxy.Director = tt.director
			}
			rproxy.Transport = tt.transport
			rproxy.CreateResponse = tt.createResponse
...
func BenchmarkServeHTTP(b *testing.B) {
	...
	proxy := &ConditionalReverseProxy{
		Director:  func(*http.Request) bool { return true },
...
func TestModifyResponseClosesBody(t *testing.T) {
	...
	rp := &ConditionalReverseProxy{
		Director: func(req *http.Request) bool { return true },
...
func TestSelectFlushInterval(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "default",
			res:  &http.Response{},
			p:    &ConditionalReverseProxy{FlushInterval: 123},
			want: 123,
		},
		{
		...
		},
		{
			...
			p:    &ConditionalReverseProxy{FlushInterval: 0},
			want: -1,
		},
	}
	for _, tt := range tests {
